...
void CFirmwareUpdateDialog::on_pushButton_connect_clicked()
{
    if(CHardwareDebug::getInstance(this)->deviceOpen())
    {
        m_deviceIsOpen = true;
        ui->pushButton_connect->setVisible(false);
        ui->label_error->setText(tr(u8"设备已连接"));
...
void CFirmwareUpdateDialog::on_buttonBox_clicked(QAbstractButton *button)
{
    if(button == ui->buttonBox->button(QDialogButtonBox::Ok))
    {
        if(checkFileName(ui->lineEdit->text()))
        {
            ...
            if(ok)
            {
                ui->label_error->setText(tr(u8"固件更新成功"));
            }
            else
            {
                ui->label_error->setText(tr(u8"固件更新失败"));
            }
